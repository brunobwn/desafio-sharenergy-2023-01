      ...
      <main className="container min-h-full px-5 mx-auto mt-4">
        <div className="flex flex-col items-center justify-between sm:flex-row">
          ...
          <form className="flex w-full max-w-md gap-2" onSubmit={(e) => handleFormSubmit(e)}>
            ...
            <button
              type="submit"
              className="px-3 py-2 font-bold tracking-wider text-white border rounded-lg shadow-sm bg-cyan opacity-90 hover:opacity-100 hover:shadow-lg focus:border-cyan focus:ring-1 focus:ring-cyan">
              GO!
            </button>
          </form>
        </div>
        <div className="mt-12">
          {errorImage !== '' ? (
            <img src={errorImage} alt="A cat" className="w-full max-w-lg mx-auto" />
          ) : null}
          {dica && (
            <p className="mt-8 text-sm text-center opacity-75">
