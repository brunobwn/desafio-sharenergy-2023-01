...
export function setUserSessionStorage(user: UserProps | null) {
  sessionStorage.setItem('uf31sa9s31sg2', JSON.stringify(user));
}
...
export function getUserSessionStorage() {
  const json = sessionStorage.getItem('uf31sa9s31sg2');
  if (json) {
    const user = JSON.parse(json);
    return user || null;
  }
